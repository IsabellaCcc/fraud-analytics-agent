...
> **TL;DR for reviewers:** This isn't just Text-to-SQL. It's a two-agent system (generator + adversarial reviewer) with conversation memory, a read-only SQL safety layer, and a rigorous evaluation framework — the patterns that matter for production LLM systems: tool use, multi-agent coordination, safety guardrails, and measurable quality.
